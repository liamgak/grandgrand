...
/**
* global selectedEventId to keep track of which event been clicked
*/
...
links.Timeline = function(container, options) {
    ...
    this.options = {
        ...
        'locale': 'en',
        'MONTHS': ["def", "January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"],
        'MONTHS_SHORT': ["def", "Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"],
        ...
        'CLUSTER_TITLE': "Cluster containing {count} events. Zoom in to see the individual events.",
        'AM': "am",
        'PM': "pm"
    };
...
links.Timeline.prototype.render = function(options) {
    ...
    //custom:highlight selected event item or cluster containing the event
    this.highlightClickedItem();
};
    ...
    if (!navBar) {
        ...
        if (showNavigation) {
            if (options.zoomable) {
                ...
                navBar.zoomInButton.className = "btn btn-default timeline-navigation-zoom-in";
                ...
                navBar.zoomOutButton.className = "btn btn-default timeline-navigation-zoom-out";
            ...
            if (options.moveable) {
                ...
                navBar.moveLeftButton.className = "btn btn-default timeline-navigation-move-left";
                ...
                navBar.moveRightButton.className = "btn btn-default timeline-navigation-move-right";
...
links.Timeline.Item = function (data, options) {
    if (data) {
        ...
        this.type = data.type;
        //custom code: adding event id that will be used to highlight specific event
...
links.Timeline.ItemBox.prototype.updateDOM = function () {
    ...
    if (divBox) {
        ...
        var eventIdClassName = 'dot-event-id-' + this.event_id;
        // update contents
        divBox.firstChild.innerHTML = this.content;

        // update class work here
        ...
        if (this.isCluster) {
            ...
            divDot.setAttribute("alt", this.title);
            divDot.setAttribute("title", this.title);
        } else {
            links.Timeline.addClassName(divDot, eventIdClassName);
        }
        ...
        if (this.className) {
         ...
         links.Timeline.addEventListener(divDot, 'keypress', function(event) {
            ...
            if(keycode == 13 && clickItemEventCallback != undefined) {
                selectedEventId = this.event_id;
                clickItemEventCallback(this.event_id, divDot);
            }
         }.bind(this));

         itemToEventMapCallback(this.event_id, divDot, this.event_ids);

         //add tabindex 0 to dot div to be accessible via keyboard
         divDot.setAttribute("tabindex", 0);

         divDot.setAttribute("role", "button");

         //let the function decide what to show in aria-label
         itemArialLabelCallback(divDot, this.className, this.start, this.content);
    }
};
...
links.Timeline.prototype.clusterItems = function () {
    ...
    var clusters = this.clusterGenerator.getClusters(this.conversion.factor, 
           this.options.clusterMaxItems, 
           this.options.CLUSTER_TITLE);
 ...
 */
links.Timeline.ClusterGenerator.prototype.getClusters = function (scale, maxItems, clusterTitleTemplate) {
    var level = -1,
            ...
            if (this.groups.hasOwnProperty(groupName)) {
                ...
                while (i < iMax) {
                    ...
                    if (neighbors > maxItems) {
                        ...
                        var cluster;
                        var title = clusterTitleTemplate.replace(/{count}/, count);
                        ...
                        if (classNames.indexOf(undefined) > -1) {
                            ...
                            selectedItemIndex = classNames.indexOf('green-m');
                        }
...
links.Timeline.StepDate.prototype.getLabelMinor = function(options, date) {
    ...
    switch (this.scale) {
        ...
        case links.Timeline.StepDate.SCALE.HOUR:
            var suffix = date.getHours() >= 12 ? options.PM : options.AM;   
            var hours = ((date.getHours() + 11) % 12 + 1);
            return hours + ":" + this.addZeros(date.getMinutes(), 2) + suffix;
...
links.Timeline.prototype.highlightClickedItem = function(){   

   if(selectedEventId != null) {

       //TODO: jquery dependent
       //clear previously selected class
       $(".timeline-event-dot").removeClass("selected");

       //highlight any item matched with the clicked item
       $(".dot-event-id-" + selectedEventId).addClass("selected");

       if(this.clusters.length){
            //highlight any cluster that has the clicked item
            for(var i = 0, size = this.clusters.length; i < size; i++){
               if(this.clusters[i].event_ids.indexOf(selectedEventId) > -1){
                 links.Timeline.addClassName(this.clusters[i].dom.dot, "selected");
                 break;
               }    
            }
       }
